...
export const currentParams: Readable<IRouteParams> = derived([params, authUser], ([rParams, currentUser], set) => {
    ...
    if (rParams.guildId !== lastGuildId && !lastGuild) {
        if (rParams.guildId) {
            ...
            res.guild = currentUser.memberGuilds.find(g => g.id === rParams.guildId) || currentUser.adminGuilds.find(g => g.id === rParams.guildId) || currentUser.modGuilds.find(g => g.id === rParams.guildId) || currentUser.bannedGuilds.find(g => g.id === rParams.guildId) || null
        } else {
            res.guild = null;
        }
    }

    if(rParams.caseId !== lastCaseId || rParams.guildId !== lastGuildId) {
