    ...
    <div class="grid grid-cols-8 gap-0 w-full lg:w-1/2">
        {#each $patchnotes as patchnote}
            {#if patchnote.text}
                <div />
                <div
                    class="flex flex-row justify-center border-solid border-2 col-span-2 px-1 py-1 mx-4"
            ...
            {:else}
                <div
                    class="border-solid border-r-2 col-span-2 pb-8 pr-4"
                    style="border-color: #161616; color: var(--cds-text-05, #6f6f6f); padding-top: 2.6rem">
                    {patchnote.released_at != undefined
                        ? moment(patchnote.released_at).format($currentLanguage?.momentDateFormat ?? "DD/MM/YYYY")
                        : "Unknown"}
                </div>
                <div class="py-8 pl-4 col-span-6">
